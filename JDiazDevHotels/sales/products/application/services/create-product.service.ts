...
export class CreateProductService implements CreateProductRequest {
    ...
    async createTheProduct(hotelId: number, dataProduct: DataProduct): Promise<any> {
        const hotel = await this.getHotelForProductDomain.getHotelForProductDomain(hotelId)

        if (!hotel) {
            return { message: "You cannot create product" }
        }

        const productCreated = await this.createProductPort.createProduct(hotelId, dataProduct)
