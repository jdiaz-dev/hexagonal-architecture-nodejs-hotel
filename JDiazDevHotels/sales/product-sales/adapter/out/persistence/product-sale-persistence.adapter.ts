...
@Service()
export class ProductSalePersistenceAdapter implements
    ...
    async createProductSaled(cashId: number, houstingId: number, productId: number, productSaleData: DataProductSaled): Promise<any> {
        const productsSaled = await this.productSaledORM.createProductSaled(cashId, houstingId, productId, productSaleData)

        return productsSaled
    }
    async updateAmountToProductSaled(productsSaledId: number, amountProducts: number, totalPrice: number): Promise<any> {
        const productsSaled = await this.productSaledORM.updateAmountToProductSaled(productsSaledId, amountProducts, totalPrice)
        return productsSaled
    }
