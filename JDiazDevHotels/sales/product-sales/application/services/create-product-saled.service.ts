...
export class CreateProductSaledService implements CreateProductSaledRequest {
    ...
    async createTheProductSale(cashId: number, houstingId: number, productId: number, productSaleData: DataProductSaled): Promise<any> {
        const product: ProductSaleDomainEntity = await this.getProductModeledForProductSaleDomain.getProductModeledForProductSaleDomain(productId)

        //bussines logic
        const totalPrice: number = product.calculateTotalPrice(productSaleData.amount)
        productSaleData.totalPrice = totalPrice

        const productSaledCreated = await this.createProductSalePort.createProductSaled(cashId, houstingId, productId, productSaleData)
