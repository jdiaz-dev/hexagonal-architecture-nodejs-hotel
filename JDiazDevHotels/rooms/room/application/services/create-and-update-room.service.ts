...
export class CreateAndUpdateRoomService implements 
    ...
    async saveDataRoom(command:RoomCommand, roomData:RoomData, action:string):Promise<any>{
        ...
        const roomDomain1 = await this.getLevelForRoomDomain.getLevelForRoomDomain(roomData.getLevelId)
        const roomDomain2 = await this.getRoomCategoryForRoomDomain.getRoomCategoryForRoomDomain(roomData.getCategoryId)
