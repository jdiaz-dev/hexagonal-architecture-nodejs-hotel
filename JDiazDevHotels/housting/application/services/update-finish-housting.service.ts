...
import { UpdateConditionOfRoomService } from "../../../rooms/room/application/services/update-condition-of-room.service";
...
export class UpdateFinishHoustingService implements FinishHoustingUseCase {
    ...
    async finishHousting(houstingId:number):Promise<any>{
        ...
        if( ! roomCondtionUpdated){
            return { message: 'A problem trying set new condition for room has ocurred'}
        }
