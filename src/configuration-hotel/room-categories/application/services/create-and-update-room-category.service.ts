...
@Service()
export class CreateAndUpdateRoomCategoryService
  implements CreateRoomCategoryRequest, UpdateRoomCategoryRequest
{
  //other domain
  ...
  async createNewRoomCategory(
    nameCategory: string,
    price: number,
    hotelId: number
  ): Promise<any> {
    const hotel =
      await this.getHotelForRoomCategoryDomain.getHotelForRoomCategoryDomain(
        hotelId
      );
    ...
    const newRoomCategory =
      await this.createRoomCategoryPort.createRoomCategory(
        nameCategory,
        price,
        hotelId
      );
  ...
  async updateTheRoomCategory(
    nameCategory: string,
    price: number,
    roomCategoryId: number,
    command: RoomCategoryCommand
  ): Promise<any> {
    const roomCategory: RoomCategoryDomainEntity =
      await this.getRoomCategoryModeledPort.getRoomCategoryModeledPort(
        roomCategoryId
      );
    ...
    const roomCatgoryUpdated =
      await this.updateRoomCategoryPort.updateCategoryRoom(
        nameCategory,
        price,
        roomCategoryId
      );
