...
export class RemoveRoomCategoryService implements RemoveRoomCategoryRequest {
  ...
  async removeTheRoomCategory(
    ...
    const roomCategoryRemoved =
      await this.removeRoomCategoryPort.removeRoomCategory(roomCategoryId);
    if (roomCategoryRemoved.state !== false) {
      return {
        message: "An error trying to remove the room category has happened",
      };
    }
