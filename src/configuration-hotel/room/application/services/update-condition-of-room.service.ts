...
@Service()
export class UpdateConditionOfRoomService
  implements UpdateCondtionOfRoomRequest, UpdateConditionFromHoustingDomain
{
  ...
  async updateTheCondtionOfRoom(
    roomId: number,
    conditionId: number
  ): Promise<any> {
    const room = await this.updateConditionOfRoomPort.updateConditionOfRoom(
      roomId,
      conditionId
    );
    return room;
  }
  async updateFromHoustingDomain(
    roomId: number,
    conditionId: number
  ): Promise<any> {
    const room = await this.updateConditionOfRoomPort.updateConditionOfRoom(
