...
export class RemoveRoomService implements RemoveTheRoomRequest {
  ...
  async removeTheRoom(
    ...
    const roomWithLevelDomain =
      await this.getRoomModelToDomainPort.getRoomModeledToDomain(roomId);
    if (
      !roomWithLevelDomain.checkIfRoomBelongsToLevel(
        roomWithLevelCommand.getLevelId
      )
    ) {
