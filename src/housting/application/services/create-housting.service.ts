...
export class CreateHoustingService {
  ...
  async createTheHousting(
    ...
    const cash = await this.getCashForHoustingDomain.getCashForHoustingDomain(
      cashId
    );
    if (!cash) {
    ...
    const client =
      await this.getClientForHoustingDomain.getClientForHoustingDomain(
        clientId
      );
    if (!client) {
    ...
    const housting = await this.createHoustingPort.createHousting(
      cashId,
      clientId,
      roomId,
      dataHousting
