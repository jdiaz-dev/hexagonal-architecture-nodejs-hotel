...
@Service()
export class SaleReportPersistenceAdapter implements CreateSaleReportPort, GetSaleReportPort {
  constructor(private saleReportORM: SaleReportORM) {}

  async createSaleReport(moneyTotal: number, houstingId: number): Promise<any> {
    const saleReport = await this.saleReportORM.createSaleReport(moneyTotal, houstingId);
    return saleReport;
  }
