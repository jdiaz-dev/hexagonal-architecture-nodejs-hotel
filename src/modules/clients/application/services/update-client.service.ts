...
export class UpdateClientService implements UpdateClientRequest {
  ...
  async updateTheClient(
    clientId: number,
    command: ClientCommand,
    dataClient: DataClient
  ): Promise<any> {
    const client: ClientDomainEntity =
      await this.getClientModeledPort.getClientModeled(clientId);

    if (!client.checkIfClientBelongsToHotel(command.getHotelId)) {
      return { message: "You cannot update this client" };
    }

    const clientUpdated = await this.updateClientPort.updateClient(
      clientId,
      dataClient
    );
