...
@Service()
export class CreateAndUpdateRoomCategoryService implements CreateRoomCategoryRequest, UpdateRoomCategoryRequest {
    //other domain
    ...
    async updateTheRoomCategory(
        ...
        const roomCategory: RoomCategoryDomainEntity =
            await this.getRoomCategoryModeledPort.getRoomCategoryModeledPort(roomCategoryId);

        if (!roomCategory.checkIfRoomCategoryBelongsToHotel(command.getHotelId)) {
            return { message: 'You cannot update this room category' };
        }

        const roomCatgoryUpdated = await this.updateRoomCategoryPort.updateCategoryRoom(
            nameCategory,
            price,
            roomCategoryId,
        );
