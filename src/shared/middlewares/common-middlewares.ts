...
export class CommonMiddlwares {
  ...
  validateJWT = async (
    ...
    try {
      const payload: any = verify(
        token,
        process.env.SECRETORPRIVATEKEY || "jdevhotels"
      );
      const user = await this.commonNeedUserFromUserBcontextPort.getUser(
        payload.id
      );

      if (!user) {
        return res.status(401).json({
          //401 : to authorized to make a specific action
  ...
  checkIfHotelBelongsToClientApp = async (
    req: Request | any,
    res: Response,
    next: NextFunction
  ) => {
    ...
    const hotel = await this.commonNeedHotelFromHotelBcontextPort.getHotel(
      hotelId
    );
