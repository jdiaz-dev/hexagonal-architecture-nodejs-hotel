...
@Service()
export class GetProductsSaledService
  implements GetProductSalesToSaleReportDomain
{
  ...
  async getProductSalesToSaleReportDomain(
    houstingId: number
  ): Promise<MoneySpentDomainEntity | boolean> {
    const productsSaled: any[] =
      await this.getProductsSaledPort.getProductsSaled(houstingId);

    if (!productsSaled) return false;

    const moneyOfSetProducts = new MoneySpentDomainEntity();
    for (let x = 0; x < productsSaled.length; x++) {
      moneyOfSetProducts.collectiontMoneySpent.push(
        productsSaled[x].totalPrice
      );
    }
