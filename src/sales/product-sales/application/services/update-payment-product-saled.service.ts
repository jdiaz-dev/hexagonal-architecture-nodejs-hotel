...
@Service()
export class UpdatePaymentProductSaledService
  implements FinishPaymentProductSaledUseCase
{
  ...
  async finishPaymentProductSaled(productSaledId: number): Promise<any> {
    const productSaledPayed =
      await this.updateProductSaledPayedPort.updateProductSaledPayed(
        productSaledId
      );
