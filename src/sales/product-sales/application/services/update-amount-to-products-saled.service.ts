...
@Service()
export class UpdateAmountToProductsSaledService
  implements UpdateAmountToProductSaledUseCase
{
  //other domains
  ...
  async updateTheAmountToProductSaled(
    productsSaledId: number,
    ammountProducts: number
  ): Promise<any> {
    const productsSaled = await this.getProductSaledPort.getProductSaled(
      productsSaledId
    );
    ...
    const product: ProductSaleDomainEntity =
      await this.getProductModeledForProductSaleDomain.getProductModeledForProductSaleDomain(
        productsSaled.productId
      );

    //bussines logic
    const totalPrice = product.calculateTotalPrice(ammountProducts);

    const newAmountProductsSaled =
      await this.updateAmountToProductSaledPort.updateAmountToProductSaled(
        productsSaledId,
        ammountProducts,
        totalPrice
      );
