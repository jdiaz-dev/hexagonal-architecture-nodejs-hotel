...
export class CreateHotelService implements CreateNewHotelRequest {
    ...
    constructor(hotelPersistenceAdapter: HotelPersistenceAdapter, getUserService: GetUserService) {
        this.createHotelPort = hotelPersistenceAdapter;
        this.hotelBcontextNeedUserFromUserBcontext = getUserService;
    }
    async createNewHotel(command: CreateHotelCommand, clientId: number, id: number): Promise<HotelModel | any> {
