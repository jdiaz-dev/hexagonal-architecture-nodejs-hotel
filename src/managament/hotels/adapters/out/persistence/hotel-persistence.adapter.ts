...
@Service()
export class HotelPersistenceAdapter implements CreateHotelPort, GetHotelPort, GetHotelByUserIdPort {
    private hotelRepository: HotelRepository;

    constructor(hotelORM: HotelORM) {
        this.hotelRepository = hotelORM;
    }
    async createHotel(dataHotel: HotelEntity, userId: number): Promise<HotelModel> {
        const hotel = {
            name: dataHotel.name,
